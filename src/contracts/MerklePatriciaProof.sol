 ...
 *         Upgrade to sol 0.5.x by Tuan Vu (tuanvd@gmail.com)
    ...
    function verify(bytes memory value, bytes memory encodedPath, bytes memory rlpParentNodes, bytes32 root) internal pure returns (bool) {
        ...
        for (uint i = 0; i < parentNodes.length; i++) {
            ...
            if (currentNodeList.length == 17) {
                ...
                nodeKey = RLPReader.toBytes32(currentNodeList[nextPathNibble]);
                pathPtr += 1;
            } else if (currentNodeList.length == 2) {
                pathPtr += _nibblesToTraverse(RLPReader.toRlpBytes(currentNodeList[0]), path, pathPtr);
                ...
                //extension node
                if (_nibblesToTraverse(RLPReader.toRlpBytes(currentNodeList[0]), path, pathPtr) == 0) {
                    return false;
                }

                nodeKey = RLPReader.toBytes32(currentNodeList[1]);
            } else {
                return false;
            }
        }
